...
const App: React.FC = () => {
    ...
    useEffect(() => {
        ...
        // Social Proof - Mostra a cada 30 segundos, dura 5 segundos
        const intervals: any[] = [];
        ...
        const socialInterval = setInterval(() => {
            ...
            try {
                ...
                sfx.volume = 1.0;
                sfx.play().catch(err => console.log('Bloqueio social proof:', err.message));
            } catch (e) {
                console.log('Erro áudio social:', e);
            }
        ...
        if (!backgroundMusicRef.current) {
            console.log('DEBUG AUDIO: Inicializando objeto de áudio...');
    ...
    // Atualizar áudio quando o estado mudar
    useEffect(() => {
        if (backgroundMusicRef.current) {
            const currentVolume = isMuted ? 0 : volume;
            backgroundMusicRef.current.volume = currentVolume;
            console.log('DEBUG AUDIO: Estado:', { isMuted, volume, audioVolume: backgroundMusicRef.current.volume });

            if (!isMuted) {
                backgroundMusicRef.current.play()
                    .then(() => console.log('DEBUG AUDIO: Play ok'))
                    .catch((err) => {
                        console.warn('DEBUG AUDIO: Play bloqueado. Interação necessária.', err.message);
                    });
            } else {
                backgroundMusicRef.current.pause();
                console.log('DEBUG AUDIO: Pause ok');
            }
        }
    }, [isMuted, volume]);
